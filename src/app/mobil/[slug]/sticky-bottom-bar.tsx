    ...
    return (
        <AnimatePresence>
            {isVisible && !isDismissed && (
                ...
                >
                    <div className="bg-white/95 backdrop-blur-2xl border-t md:border border-slate-100 md:rounded-full shadow-[0_-15px_50px_-10px_rgba(0,0,0,0.15)] md:shadow-2xl px-3 py-3 md:px-10 md:py-4 flex flex-col md:flex-row items-stretch md:items-center gap-4 md:gap-8 w-full md:w-auto">

                        {/* Mobile Info View (Price & Model) */}
                        <div className="flex md:hidden items-center justify-between px-2 pb-0.5">
                            <div>
                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Mulai Dari</p>
                                <p className="text-sm font-black text-slate-900 tracking-tighter">
                                    {new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(carPrice)}
                                </p>
                            </div>
                            <div className="text-right">
                                <p className="text-[9px] font-black text-red-600 uppercase tracking-widest mb-0.5">Unit Ready Stock</p>
                                <p className="text-[11px] font-black text-slate-900 uppercase tracking-tighter">{carName}</p>
                            </div>
                        </div>

                        <div className="flex items-center justify-between md:justify-center gap-1 md:gap-8">
                            {/* Hubungi Sales */}
                            <button
                            ...
                            >
                                ...
                                <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-slate-500 group-hover:text-slate-900 whitespace-nowrap">OFFER</span>
                            </button>
                            ...
                            <button
                                onClick={async () => {
                                    if (navigator.share) {
                                        await navigator.share({ title: carName, url: window.location.href });
                                    } else {
                            ...
                            <button
                                ...
                                className="flex-[2] md:flex-none flex items-center justify-center gap-2 md:gap-3 px-5 py-4 md:px-8 md:py-4 bg-red-600 rounded-2xl md:rounded-full text-white shadow-lg shadow-red-600/30 hover:bg-red-700 transition-all hover:scale-105 active:scale-95 group"
                            >
                                <CheckCircle2 className="h-4 w-4 md:h-5 md:w-5" />
                                <span className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.2em] whitespace-nowrap">BELI SEKARANG</span>
                            </button>
                        </div>
